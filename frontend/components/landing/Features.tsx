...
export default function Features() {
  ...
  return (
    <section
      id="features"
      className="py-20 md:py-32 w-full relative bg-background"
    >
      {/* Background decorations */}
      <div className="absolute inset-0">
        <div className="absolute top-20 left-10 w-32 h-32 bg-secondary/10 rounded-full blur-2xl"></div>
      ...
      <div className="container px-4 mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {features.map((feature, index) => (
            ...
            >
              <Card
                className="group h-full relative overflow-hidden transition-all duration-500 hover:shadow-2xl hover:shadow-secondary/10 border-2 hover:border-secondary/30 cursor-pointer bg-card"
                onMouseEnter={() => setHoveredIndex(index)}
                onMouseLeave={() => setHoveredIndex(null)}
                onClick={handleFeatureClick}
              >
                {/* Animated background gradient */}
                <div className="absolute inset-0 bg-gradient-to-br from-background to-secondary opacity-0 group-hover:opacity-5 transition-opacity duration-500"></div>
                ...
                <CardContent className="p-6 relative z-10">
                  <div className="flex items-center mb-4">
                    <div className="p-3 rounded-xl bg-gradient-to-br from-primary to-foreground text-primary-foreground transform transition-all duration-300 group-hover:scale-110 group-hover:rotate-3">
                  ...
                  <div
                    className={`flex items-center text-primary font-medium transition-all duration-300 ${
                      hoveredIndex === index
